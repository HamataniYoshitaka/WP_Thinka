<?php
/**
 * horikenテンプレートの参考価格ページのファイルです。
 *
 * @since horiken 1.0.0
 */

get_header();
?>

<main class="price-page">
  <?php
get_template_part( 'includes/header' );
?>


  <section class="section-mv">
    <div class="container">
      <p class="page-title">
        <span class="english">Large facility</span>
        大型施設
      </p>
      <div class="box-overview">
        <h1 class="title">
          建築費用参考価格
        </h1>
        <p class="overview">システム建築による倉庫参考お見積り価格を紹介します。こちらで紹介しているお見積りは商品の一例となります。</p>
      </div>
    </div>
  </section>


  <section class="section-contents">
    <div class="container">
      <ul id="tab-menu" class="tab-menu">
        <li id="tab1"><a href="#64">64坪 <br class="xs">(210.6㎡)</a></li>
        <li id="tab2"><a href="#271">271坪  <br class="xs">(894.24㎡)</a></li>
        <li id="tab3"><a href="#401">401坪 <br class="xs">(1321.92㎡)</a></li>
        <li id="tab4"><a href="#559">559坪 <br class="xs"> (1845.18㎡) </a></li>
      </ul>

      <div class="tab-panel" id="64">
        <div class="box-estimate">
          <div class="box-price">
            <p class="title">
              お見積もり金額
            </p>
            <p class="price">￥20,880,000‐<span class="tax">(税別)</span></p>
            <p class="notice">
              ※2023年8月参考価格
            </p>
          </div>
          <div class="box-conditions">
            <ul class="list-condition">
              <li class="condition">
                <p class="label">建物金額</p>
                <p class="data">￥13,810,000‐(税別)</p>
              </li>
              <li class="condition">
                <p class="label">基礎金額</p>
                <p class="data">￥7,070,000‐(税別)</p>
              </li>
              <li class="condition">
                <p class="label">建物寸法</p>
                <p class="data">幅11.7m × 長さ18m × 高さ4m</p>
              </li>
              <li class="condition">
                <p class="label">建物面積</p>
                <p class="data">210.6㎡　(単価 ￥99,100/㎡)<br>64坪　(単価 ￥326,200/坪)</p>
              </li>
            </ul>
            <p class="list-title">
              建物仕様
            </p>
            <ul class="list-condition">
              <li class="condition">
                <p class="label">主構造</p>
                <p class="data">鉄骨造yessテーパーフレーム錆止め塗装（グレー色）</p>
              </li>
              <li class="condition">
                <p class="label">外装</p>
                <p class="data">屋根 SSルーフガルバリウム鋼板素地</p>
              </li>
              <li class="condition">
                <p class="label">外壁</p>
                <p class="data">Vリブウォールカラーガルバリウム鋼板</p>
              </li>
              <li class="condition">
                <p class="label">断熱</p>
                <p class="data">屋根 YマットS表皮付きグラスウール60mm</p>
              </li>
              <li class="condition">
                <p class="label">外壁</p>
                <p class="data">プラスターボード 12.5mm</p>
              </li>
              <li class="condition">
                <p class="label">建具</p>
                <p class="data">
                  オーバースライダー:1箇所（W4100mm × H3000mm, 水圧解錠装置込）<br>
                  引き違い窓：2箇所 （W3500mm × H1215mm, ガラス型4mm）<br>
                  スチールドア：１箇所 （W 770mm × H2000mm, 窓付き）</p>
              </li>
            </ul>
            <p class="list-title">
              注記
            </p>
            <ul class="list-condition">
              <li class="condition">
                <p class="label">工事範囲</p>
                <p class="data">(1)鉄骨工事 (2)屋根工事 (3)外装工事 (4)外部標準建具工事 (5)樋工事 (6)側足場 (7)基礎工事<br>※基礎工事は「基礎想定仕様：直接基礎(地耐力
                  5t/㎡)＋土間(12㎝厚)」を想定しています。</p>
              </li>
              <li class="condition">
                <p class="label">別途工事</p>
                <p class="data">次の工事・経費等は上記金額に含みません。<br>電気工事 設備工事 給排水工事 排煙装置衛生工事 内装工事 外構工事<br>共通及び直接仮設工事 建築申請費用</p>
              </li>
              <li class="condition">
                <p class="label">建設地域</p>
                <p class="data">基本風速が38m/secを超える地域や沖縄県・離島、その他特殊な建設地には適用できません。</p>
              </li>
              <li class="condition">
                <p class="label">その他</p>
                <p class="data">
                  建物寸法及び条件が下記に該当する場合は別途見積りをご依頼ください。<br>建物幅が30.6mを超える場合、建物長が60.3m超える場合、軒高が9m超える場合、<br>屋根ベンチレーターがある場合、天井走行クレーンが設置される場合
                </p>
              </li>
            </ul>
            <p class="list-title">
              見積指定条件
            </p>
            <ul class="list-condition">
              <li class="condition">
                <p class="label">基準風速</p>
                <p class="data">34m/sec</p>
              </li>
              <li class="condition">
                <p class="label">粗度区分</p>
                <p class="data">Ⅲ</p>
              </li>
              <li class="condition">
                <p class="label">積雪荷重</p>
                <p class="data">40cm X 2kg X 100%＜大阪府＞</p>
              </li>
            </ul>
          </div>
        </div>
        <ul class="list-image">
          <li class="image">
            <img src="<?php echo get_template_directory_uri(); ?>/assets/imgs/price/minami.png" alt="" class="image">
            <p class="text">南側</p>
          </li>
             <li class="image">
            <img src="<?php echo get_template_directory_uri(); ?>/assets/imgs/price/higashi.png" alt="" class="image">
            <p class="text">東側</p>
          </li>
             <li class="image">
            <img src="<?php echo get_template_directory_uri(); ?>/assets/imgs/price/kita.png" alt="" class="image">
            <p class="text">北側</p>
          </li>
             <li class="image">
            <img src="<?php echo get_template_directory_uri(); ?>/assets/imgs/price/nishi.png" alt="" class="image">
            <p class="text">西側</p>
          </li>
             <li class="image">
            <img src="<?php echo get_template_directory_uri(); ?>/assets/imgs/price/nanto.png" alt="" class="image">
            <p class="text">南東側</p>
          </li>
             <li class="image">
            <img src="<?php echo get_template_directory_uri(); ?>/assets/imgs/price/hokusei.png" alt="" class="image">
            <p class="text">北西側</p>
          </li>
             <li class="image">
            <img src="<?php echo get_template_directory_uri(); ?>/assets/imgs/price/naka.png" alt="" class="image">
            <p class="text">内観</p>
          </li>
             <li class="image">
            <img src="<?php echo get_template_directory_uri(); ?>/assets/imgs/price/naka2.png" alt="" class="image">
            <p class="text">内観</p>
          </li>
        </ul>
      </div>


      <div class="tab-panel" id="271">
        <div class="box-estimate">
          <div class="box-price">
            <p class="title">
              お見積もり金額
            </p>
            <p class="price">￥61,390,000‐<span class="tax">(税別)</span></p>
            <p class="notice">
              ※2023年8月参考価格
            </p>
          </div>
          <div class="box-conditions">
            <ul class="list-condition">
              <li class="condition">
                <p class="label">建物金額</p>
                <p class="data">￥39,730,000‐(税別)</p>
              </li>
              <li class="condition">
                <p class="label">基礎金額</p>
                <p class="data">￥21,660,000‐(税別)</p>
              </li>
              <li class="condition">
                <p class="label">建物寸法</p>
                <p class="data">幅20.7m × 長さ43.2m × 高さ4m</p>
              </li>
              <li class="condition">
                <p class="label">建物面積</p>
                <p class="data">894.24㎡ (単価 ￥68,600/㎡)<br />271坪 (単価 ￥226,500/坪)</p>
              </li>
            </ul>
            <p class="list-title">
              建物仕様
            </p>
            <ul class="list-condition">
              <li class="condition">
                <p class="label">主構造</p>
                <p class="data">鉄骨造yessテーパーフレーム錆止め塗装（グレー色）</p>
              </li>
              <li class="condition">
                <p class="label">外装</p>
                <p class="data">屋根 SSルーフガルバリウム鋼板素地</p>
              </li>
              <li class="condition">
                <p class="label">外壁</p>
                <p class="data">Vリブウォールカラーガルバリウム鋼板</p>
              </li>
              <li class="condition">
                <p class="label">断熱</p>
                <p class="data">屋根 YマットS表皮付きグラスウール60mm</p>
              </li>
              <li class="condition">
                <p class="label">外壁</p>
                <p class="data">プラスターボード 12.5mm</p>
              </li>
              <li class="condition">
                <p class="label">建具</p>
                <p class="data">
                  オーバースライダー:2箇所（W4100mm × H3000mm, 水圧解錠装置込）<br />
                  引き違い窓:9箇所（W3500mm × H1215mm, ガラス型4mm）<br />
                  スチールドア:2箇所（W 770mm × H2000mm, 窓付き）
                </p>
              </li>
            </ul>
            <p class="list-title">
              注記
            </p>
            <ul class="list-condition">
              <li class="condition">
                <p class="label">工事範囲</p>
                <p class="data">(1)鉄骨工事 (2)屋根工事 (3)外装工事 (4)外部標準建具工事 (5)樋工事 (6)側足場 (7)基礎工事<br>※基礎工事は「基礎想定仕様：直接基礎(地耐力
                  5t/㎡)＋土間(12㎝厚)」を想定しています。</p>
              </li>
              <li class="condition">
                <p class="label">別途工事</p>
                <p class="data">次の工事・経費等は上記金額に含みません。<br>電気工事 設備工事 給排水工事 排煙装置衛生工事 内装工事 外構工事<br>共通及び直接仮設工事 建築申請費用</p>
              </li>
              <li class="condition">
                <p class="label">建設地域</p>
                <p class="data">基本風速が38m/secを超える地域や沖縄県・離島、その他特殊な建設地には適用できません。</p>
              </li>
              <li class="condition">
                <p class="label">その他</p>
                <p class="data">
                  建物寸法及び条件が下記に該当する場合は別途見積りをご依頼ください。<br>建物幅が30.6mを超える場合、建物長が60.3m超える場合、軒高が9m超える場合、<br>屋根ベンチレーターがある場合、天井走行クレーンが設置される場合
                </p>
              </li>
            </ul>
            <p class="list-title">
              見積指定条件
            </p>
            <ul class="list-condition">
              <li class="condition">
                <p class="label">基準風速</p>
                <p class="data">34m/sec</p>
              </li>
              <li class="condition">
                <p class="label">粗度区分</p>
                <p class="data">Ⅲ</p>
              </li>
              <li class="condition">
                <p class="label">積雪荷重</p>
                <p class="data">40cm X 2kg X 100%＜大阪府＞</p>
              </li>
            </ul>
          </div>
        </div>
        <ul class="list-image">
          <li class="image">
            <img src="<?php echo get_template_directory_uri(); ?>/assets/imgs/price/minami.png" alt="" class="image">
            <p class="text">南側</p>
          </li>
             <li class="image">
            <img src="<?php echo get_template_directory_uri(); ?>/assets/imgs/price/higashi.png" alt="" class="image">
            <p class="text">東側</p>
          </li>
             <li class="image">
            <img src="<?php echo get_template_directory_uri(); ?>/assets/imgs/price/kita.png" alt="" class="image">
            <p class="text">北側</p>
          </li>
             <li class="image">
            <img src="<?php echo get_template_directory_uri(); ?>/assets/imgs/price/nishi.png" alt="" class="image">
            <p class="text">西側</p>
          </li>
             <li class="image">
            <img src="<?php echo get_template_directory_uri(); ?>/assets/imgs/price/nanto.png" alt="" class="image">
            <p class="text">南東側</p>
          </li>
             <li class="image">
            <img src="<?php echo get_template_directory_uri(); ?>/assets/imgs/price/hokusei.png" alt="" class="image">
            <p class="text">北西側</p>
          </li>
             <li class="image">
            <img src="<?php echo get_template_directory_uri(); ?>/assets/imgs/price/naka.png" alt="" class="image">
            <p class="text">内観</p>
          </li>
             <li class="image">
            <img src="<?php echo get_template_directory_uri(); ?>/assets/imgs/price/naka2.png" alt="" class="image">
            <p class="text">内観</p>
          </li>
        </ul>
      </div>
      <div class="tab-panel" id="401">
        <div class="box-estimate">
          <div class="box-price">
            <p class="title">
              お見積もり金額
            </p>
            <p class="price">￥88,240,000‐<span class="tax">(税別)</span></p>
            <p class="notice">
              ※2023年8月参考価格
            </p>
          </div>
          <div class="box-conditions">
            <ul class="list-condition">
              <li class="condition">
                <p class="label">建物金額</p>
                <p class="data">￥58,880,000‐(税別)</p>
              </li>
              <li class="condition">
                <p class="label">基礎金額</p>
                <p class="data">￥29,360,000‐(税別)</p>
              </li>
              <li class="condition">
                <p class="label">建物寸法</p>
                <p class="data">幅30.6m × 長さ43.2m × 高さ4m</p>
              </li>
              <li class="condition">
                <p class="label">建物面積</p>
                <p class="data">1321.92㎡(単価 ￥66,700/㎡)<br />401坪(単価 ￥220,000/坪)</p>
              </li>
            </ul>
            <p class="list-title">
              建物仕様
            </p>
            <ul class="list-condition">
              <li class="condition">
                <p class="label">主構造</p>
                <p class="data">鉄骨造yessテーパーフレーム錆止め塗装（グレー色）</p>
              </li>
              <li class="condition">
                <p class="label">外装</p>
                <p class="data">屋根 SSルーフガルバリウム鋼板素地</p>
              </li>
              <li class="condition">
                <p class="label">外壁</p>
                <p class="data">Vリブウォールカラーガルバリウム鋼板</p>
              </li>
              <li class="condition">
                <p class="label">断熱</p>
                <p class="data">屋根 YマットS表皮付きグラスウール60mm</p>
              </li>
              <li class="condition">
                <p class="label">外壁</p>
                <p class="data">プラスターボード 12.5mm</p>
              </li>
              <li class="condition">
                <p class="label">建具</p>
                <p class="data">
                  オーバースライダー:3 箇所（W4100mm × H3000mm, 水圧解錠装置込）<br />
                  引き違い窓:12 箇所（W3500mm × H1215mm, ガラス型4mm）<br />
                  スチールドア:3箇所（W 770mm × H2000mm, 窓付き）
                </p>
              </li>
            </ul>
            <p class="list-title">
              注記
            </p>
            <ul class="list-condition">
              <li class="condition">
                <p class="label">工事範囲</p>
                <p class="data">(1)鉄骨工事 (2)屋根工事 (3)外装工事 (4)外部標準建具工事 (5)樋工事 (6)側足場 (7)基礎工事<br>※基礎工事は「基礎想定仕様：直接基礎(地耐力
                  5t/㎡)＋土間(12㎝厚)」を想定しています。</p>
              </li>
              <li class="condition">
                <p class="label">別途工事</p>
                <p class="data">次の工事・経費等は上記金額に含みません。<br>電気工事 設備工事 給排水工事 排煙装置衛生工事 内装工事 外構工事<br>共通及び直接仮設工事 建築申請費用</p>
              </li>
              <li class="condition">
                <p class="label">建設地域</p>
                <p class="data">基本風速が38m/secを超える地域や沖縄県・離島、その他特殊な建設地には適用できません。</p>
              </li>
              <li class="condition">
                <p class="label">その他</p>
                <p class="data">
                  建物寸法及び条件が下記に該当する場合は別途見積りをご依頼ください。<br>建物幅が30.6mを超える場合、建物長が60.3m超える場合、軒高が9m超える場合、<br>屋根ベンチレーターがある場合、天井走行クレーンが設置される場合
                </p>
              </li>
            </ul>
            <p class="list-title">
              見積指定条件
            </p>
            <ul class="list-condition">
              <li class="condition">
                <p class="label">基準風速</p>
                <p class="data">34m/sec</p>
              </li>
              <li class="condition">
                <p class="label">粗度区分</p>
                <p class="data">Ⅲ</p>
              </li>
              <li class="condition">
                <p class="label">積雪荷重</p>
                <p class="data">40cm X 2kg X 100%＜大阪府＞</p>
              </li>
            </ul>
          </div>
        </div>
        <ul class="list-image">
          <li class="image">
            <img src="<?php echo get_template_directory_uri(); ?>/assets/imgs/price/minami.png" alt="" class="image">
            <p class="text">南側</p>
          </li>
             <li class="image">
            <img src="<?php echo get_template_directory_uri(); ?>/assets/imgs/price/higashi.png" alt="" class="image">
            <p class="text">東側</p>
          </li>
             <li class="image">
            <img src="<?php echo get_template_directory_uri(); ?>/assets/imgs/price/kita.png" alt="" class="image">
            <p class="text">北側</p>
          </li>
             <li class="image">
            <img src="<?php echo get_template_directory_uri(); ?>/assets/imgs/price/nishi.png" alt="" class="image">
            <p class="text">西側</p>
          </li>
             <li class="image">
            <img src="<?php echo get_template_directory_uri(); ?>/assets/imgs/price/nanto.png" alt="" class="image">
            <p class="text">南東側</p>
          </li>
             <li class="image">
            <img src="<?php echo get_template_directory_uri(); ?>/assets/imgs/price/hokusei.png" alt="" class="image">
            <p class="text">北西側</p>
          </li>
             <li class="image">
            <img src="<?php echo get_template_directory_uri(); ?>/assets/imgs/price/naka.png" alt="" class="image">
            <p class="text">内観</p>
          </li>
             <li class="image">
            <img src="<?php echo get_template_directory_uri(); ?>/assets/imgs/price/naka2.png" alt="" class="image">
            <p class="text">内観</p>
          </li>
        </ul>
      </div>
      <div class="tab-panel" id="559">
        <div class="box-estimate">
          <div class="box-price">
            <p class="title">
              お見積もり金額
            </p>
            <p class="price">￥118,940,000‐<span class="tax">(税別)</span></p>
            <p class="notice">
              ※2023年8月参考価格
            </p>
          </div>
          <div class="box-conditions">
            <ul class="list-condition">
              <li class="condition">
                <p class="label">建物金額</p>
                <p class="data">￥78,920,000‐(税別)</p>
              </li>
              <li class="condition">
                <p class="label">基礎金額</p>
                <p class="data">￥40,020,000‐(税別)</p>
              </li>
              <li class="condition">
                <p class="label">建物寸法</p>
                <p class="data">幅30.6m × 長さ60.3m × 高さ4m</p>
              </li>
              <li class="condition">
                <p class="label">建物面積</p>
                <p class="data">1845.18㎡(単価 ￥64,400/㎡)<br />559坪(単価 ￥212,700/坪)</p>
              </li>
            </ul>
            <p class="list-title">
              建物仕様
            </p>
            <ul class="list-condition">
              <li class="condition">
                <p class="label">主構造</p>
                <p class="data">鉄骨造yessテーパーフレーム錆止め塗装（グレー色）</p>
              </li>
              <li class="condition">
                <p class="label">外装</p>
                <p class="data">屋根 SSルーフガルバリウム鋼板素地</p>
              </li>
              <li class="condition">
                <p class="label">外壁</p>
                <p class="data">Vリブウォールカラーガルバリウム鋼板</p>
              </li>
              <li class="condition">
                <p class="label">断熱</p>
                <p class="data">屋根 YマットS表皮付きグラスウール60mm</p>
              </li>
              <li class="condition">
                <p class="label">外壁</p>
                <p class="data">プラスターボード 12.5mm</p>
              </li>
              <li class="condition">
                <p class="label">建具</p>
                <p class="data">
                  オーバースライダー:4箇所（W4100mm × H3000mm, 水圧解錠装置込）<br />
                  引き違い窓:16 箇所（W3500mm × H1215mm, ガラス型4mm）<br />
                  スチールドア:4箇所（W 770mm × H2000mm, 窓付き）
                </p>
              </li>
            </ul>
            <p class="list-title">
              注記
            </p>
            <ul class="list-condition">
              <li class="condition">
                <p class="label">工事範囲</p>
                <p class="data">(1)鉄骨工事 (2)屋根工事 (3)外装工事 (4)外部標準建具工事 (5)樋工事 (6)側足場 (7)基礎工事<br>※基礎工事は「基礎想定仕様：直接基礎(地耐力
                  5t/㎡)＋土間(12㎝厚)」を想定しています。</p>
              </li>
              <li class="condition">
                <p class="label">別途工事</p>
                <p class="data">次の工事・経費等は上記金額に含みません。<br>電気工事 設備工事 給排水工事 排煙装置衛生工事 内装工事 外構工事<br>共通及び直接仮設工事 建築申請費用</p>
              </li>
              <li class="condition">
                <p class="label">建設地域</p>
                <p class="data">基本風速が38m/secを超える地域や沖縄県・離島、その他特殊な建設地には適用できません。</p>
              </li>
              <li class="condition">
                <p class="label">その他</p>
                <p class="data">
                  建物寸法及び条件が下記に該当する場合は別途見積りをご依頼ください。<br>建物幅が30.6mを超える場合、建物長が60.3m超える場合、軒高が9m超える場合、<br>屋根ベンチレーターがある場合、天井走行クレーンが設置される場合
                </p>
              </li>
            </ul>
            <p class="list-title">
              見積指定条件
            </p>
            <ul class="list-condition">
              <li class="condition">
                <p class="label">基準風速</p>
                <p class="data">34m/sec</p>
              </li>
              <li class="condition">
                <p class="label">粗度区分</p>
                <p class="data">Ⅲ</p>
              </li>
              <li class="condition">
                <p class="label">積雪荷重</p>
                <p class="data">40cm X 2kg X 100%＜大阪府＞</p>
              </li>
            </ul>
          </div>
        </div>
        <ul class="list-image">
          <li class="image">
            <img src="<?php echo get_template_directory_uri(); ?>/assets/imgs/price/minami.png" alt="" class="image">
            <p class="text">南側</p>
          </li>
             <li class="image">
            <img src="<?php echo get_template_directory_uri(); ?>/assets/imgs/price/higashi.png" alt="" class="image">
            <p class="text">東側</p>
          </li>
             <li class="image">
            <img src="<?php echo get_template_directory_uri(); ?>/assets/imgs/price/kita.png" alt="" class="image">
            <p class="text">北側</p>
          </li>
             <li class="image">
            <img src="<?php echo get_template_directory_uri(); ?>/assets/imgs/price/nishi.png" alt="" class="image">
            <p class="text">西側</p>
          </li>
             <li class="image">
            <img src="<?php echo get_template_directory_uri(); ?>/assets/imgs/price/nanto.png" alt="" class="image">
            <p class="text">南東側</p>
          </li>
             <li class="image">
            <img src="<?php echo get_template_directory_uri(); ?>/assets/imgs/price/hokusei.png" alt="" class="image">
            <p class="text">北西側</p>
          </li>
             <li class="image">
            <img src="<?php echo get_template_directory_uri(); ?>/assets/imgs/price/naka.png" alt="" class="image">
            <p class="text">内観</p>
          </li>
             <li class="image">
            <img src="<?php echo get_template_directory_uri(); ?>/assets/imgs/price/naka2.png" alt="" class="image">
            <p class="text">内観</p>
          </li>
        </ul>
      </div>
    </div>
  </section>



  <?php
get_template_part( 'includes/flow' );
?>
  <?php
get_template_part( 'includes/contact' );
?>
  <?php
get_template_part( 'includes/footer' );
?>

</main><!-- #site-content -->
<?php
get_footer();
?>